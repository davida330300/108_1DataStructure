...
HashTableChained<K, V>::HashTableChained(int sizeEstimate)
{
    ...
    int sizeBucket = sizeEstimate;

    if (!isPrime(sizeBucket))
    {
        while (!isPrime(sizeBucket))
        {
            sizeBucket++;
        }
    }
    table = new DList<Entry<K, V>>[sizeBucket];
    entrysize = 0;
}
...
int HashTableChained<K, V>::compFunction(int code)
{
    ...
    int compResult = (abs(127 * code + 97) % 16908799) % tablesize;
    return compResult;
}
...
void HashTableChained<K, V>::insert(const K &key, const V &value)
{
    ...
    table[compFunction(key)].insertEnd(add);
    entrysize++;
    return;
...
{
    // Replace the following line with your solution.
    int index = compFunction(key);
    while(table[index]->next){
        if(table[index]->)
    }
}
